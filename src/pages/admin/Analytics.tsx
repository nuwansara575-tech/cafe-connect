...
const COLORS = ["hsl(142, 76%, 36%)", "hsl(22, 97%, 54%)", "hsl(0, 0%, 60%)"];

export default function Analytics() {
  const [loading, setLoading] = useState(true);
  const [statusData, setStatusData] = useState<{ name: string; value: number }[]>([]);
  const [dailyRedemptions, setDailyRedemptions] = useState<{ date: string; count: number }[]>([]);
  const [campaignData, setCampaignData] = useState<{ name: string; coupons: number; redeemed: number }[]>([]);

  useEffect(() => {
    async function load() {
      // Status breakdown
      const { data: coupons } = await supabase.from("coupons").select("status, campaign_name, redeemed_at");
      const statuses = { unused: 0, redeemed: 0, expired: 0 };
      const campMap: Record<string, { coupons: number; redeemed: number }> = {};
      ...
      coupons?.forEach(c => {
        ...
        if (!campMap[cn]) campMap[cn] = { coupons: 0, redeemed: 0 };
        campMap[cn].coupons++;
      ...
      setDailyRedemptions(Object.entries(dayMap).map(([date, count]) => ({ date, count })));
